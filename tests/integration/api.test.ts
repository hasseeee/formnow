...
describe('認証', () => {
  it('管理APIはトークンなし・誤ったトークンを拒否する', async () => {
    ...
    expect(
      (
        await client.request('GET', '/api/admin/events', undefined, {
  ...
  it('MCPはトークンなし・管理トークンでは使えない', async () => {
    ...
    expect(
      (
        await client.request('POST', '/mcp', init, {
          ...accept,
  ...
  it('シークレットが未設定なら、どんなトークンでも拒否する', async () => {
    ...
    expect(
      (
        await unset.request('GET', '/api/admin/events', undefined, {
          Authorization: 'Bearer undefined',
        })
      ).status,
    ).toBe(401);
    expect(
      (await unset.request('GET', '/api/admin/events', undefined, { Authorization: 'Bearer ' }))
        .status,
    ).toBe(401);
    expect((await unset.request('POST', '/mcp', {}, { Authorization: 'Bearer ' })).status).toBe(
      401,
    );
  });
});
...
describe('回答者向けのフォーム取得', () => {
  it('重みと選択肢の配点を含めない', async () => {
    ...
    expect(view.questions[1].options).toEqual([
      { label: 'ぜひ' },
      { label: 'まあ' },
      { label: 'うーん' },
    ]);
  ...
  it('フォームの種別に合う回答者だけを返す', async () => {
    const judge = (await (
      await client.request('GET', '/api/forms/judge')
    ).json()) as PublicFormView;
  ...
  it('下書きは回答者には見えないが、管理者のプレビューでは見える', async () => {
    ...
    const preview = await client.adminJson<PublicFormView>(
      'GET',
...
describe('回答の送信', () => {
  ...
  it('不正な入力を拒否する', async () => {
    const post = (slug: string, body: unknown) =>
      client.request('POST', `/api/forms/${slug}/responses`, body);
    ...
    const peerAnswers = [
      { questionId: f.pq.like.id, value: 5 },
      { questionId: f.pq.again.id, value: 'ぜひ' },
    ];
    expect((await post('peer', { respondentId: j, teamId: a, answers: peerAnswers })).status).toBe(
      403,
    );
    expect((await submitJudge(client, f, f.memberA.id, a, 5, 5)).status).toBe(403);
    // 自分のチームを評価
    expect(
      (await post('peer', { respondentId: f.memberA.id, teamId: a, answers: peerAnswers })).status,
    ).toBe(403);
    // 必須の欠落・空文字
    expect(
      (
        await post('judge', {
          respondentId: j,
          teamId: a,
          answers: [{ questionId: tech, value: 8 }],
        })
      ).status,
    ).toBe(400);
    // 配点上限の超過・負の値
    ...
    // 同じ質問への重複回答
    const dup = [
      { questionId: tech, value: 1 },
      { questionId: tech, value: 2 },
      { questionId: design, value: 3 },
    ];
    expect((await post('judge', { respondentId: j, teamId: a, answers: dup })).status).toBe(400);
    // 選択肢にない値
    const badChoice = [
      { questionId: f.pq.like.id, value: 5 },
      { questionId: f.pq.again.id, value: '最高' },
    ];
    expect(
      (await post('peer', { respondentId: f.memberA.id, teamId: f.teams.b.id, answers: badChoice }))
        .status,
    ).toBe(400);
    // 存在しないチーム・別フォームの質問
    expect((await submitJudge(client, f, j, 9999, 5, 5)).status).toBe(400);
    const foreign = [
      { questionId: tech, value: 5 },
      { questionId: design, value: 5 },
      { questionId: f.pq.like.id, value: 5 },
    ];
    expect((await post('judge', { respondentId: j, teamId: a, answers: foreign })).status).toBe(
      400,
    );
...
describe('集計', () => {
  beforeEach(async () => {
    ...
    await client.request('POST', '/api/forms/peer/responses', {
      ...
      teamId: f.teams.a.id,
      answers: [
        { questionId: f.pq.like.id, value: 4 },
        { questionId: f.pq.again.id, value: 'ぜひ' },
      ], // 6
    });
    await client.request('POST', '/api/forms/peer/responses', {
      ...
      teamId: f.teams.b.id,
      answers: [
        { questionId: f.pq.like.id, value: 3 },
        { questionId: f.pq.again.id, value: 'うーん' },
      ], // 3
    });
  });
  ...
  it('計算式で複数フォームを合算し、回答のないチームは0点ではなく「なし」になる', async () => {
    ...
    const ranking = formulas[0].ranking.map((r) => [
      r.teamName,
...
describe('データを消さないための約束', () => {
  ...
  it('MCPのマージ: 質問を渡し直しても、既存の質問と回答値は残る', async () => {
    ...
    expect(questions.map((q) => q.labelMd).sort()).toEqual(
      ['デザイン', '技術力', '発表', '講評'].sort(),
    );
    expect(questions.find((q) => q.labelMd === '技術力')).toMatchObject({
      id: f.jq.tech.id,
      weight: 3,
    });
    const summary = await client.adminJson<FormSummary>(
  ...
  it('回答者の所属に、別のイベントのチームは指定できない', async () => {
    const other = await client.adminJson<{ id: number }>('POST', '/api/admin/events', {
      name: '別イベント',
    });
    const [otherTeam] = await client.adminJson<{ id: number }[]>(
      'PUT',
      `/api/admin/events/${other.id}/teams`,
      {
...
describe('フォームの作成', () => {
  ...
  it('URL名の重複は409', async () => {
    const res = await client.admin('POST', '/api/admin/forms', {
      eventId: f.eventId,
      slug: 'judge',
      title: 'x',
      kind: 'judge',
