...
2. **チーム別集計**（フォーム単位）: avg / sum / count、質問別平均、順位
   - 回答者ごとの傾向（件数・平均・全体との差・ばらつき）と、回答者ごとに平均0・ばらつき1にそろえたスコアのチーム平均＝標準化平均・標準化順位を併記（回答1件・全部同じ点の回答者は標準化から除外。詳細は [2026-09-23-judge-calibration.md](2026-09-23-judge-calibration.md)）
3. **自由計算式**（イベント単位）: 変数 `<form_slug>_avg` `<form_slug>_sum` `<form_slug>_count` `<form_slug>_zavg`（標準化平均。null のチームは未登録なので結果 null）、四則演算・括弧・数値リテラル。evalは使わず自前トークナイザ＋再帰下降パーサー（変数名は既知変数の最長一致で切り出し、ハイフン付きslugと減算を両立）。ゼロ除算・構文エラーは式のエラーとして表示。回答0件のフォームのavgを参照したチームは結果null（0点扱いしない）
